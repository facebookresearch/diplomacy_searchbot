...
def _create_checkpoint() -> pathlib.Path:
...
def _get_or_create_checkpoint_for_out_dir(out_dir: pathlib.Path) -> pathlib.Path:
...
def handle_parser_arg(checkpoint_arg: str, out_dir: pathlib.Path) -> pathlib.Path:
    ...
    if checkpoint_arg and checkpoint_arg != NONE_CHECKPOINT and not util.is_on_slurm():
        if checkpoint_arg == DEFAULT_CHECKPOINT:
            ckpt_dir = _get_or_create_checkpoint_for_out_dir(out_dir)
        else:
            ckpt_dir = pathlib.Path(checkpoint_arg)
            assert ckpt_dir.exists(), checkpoint_arg
    else:
