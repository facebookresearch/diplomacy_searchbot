...
void SinglePowerCFRStats::update(double state_utility,
  ...
  // Recompute the next probabilites of actions, CFR1P (regret matching)
  else {
...
void SinglePowerCFRStats::prune_action_idxs(
    const std::vector<int> &action_idxs) {
  if (action_idxs.size() <= 0)
    return;

  std::set<int> action_idx_set;
  for (int action_idx : action_idxs) {
    JCHECK(action_idx >= 0 && action_idx < actions_.size(),
           "Invalid action index to prune");
    action_idx_set.insert(action_idx);
  }
  JCHECK(action_idx_set.size() < actions_.size(),
         "Attempted to prune all possible actions");

  // Rebuild actions, dropping all the ones we want to prune
  std::vector<ActionData> prunedActions;
  for (int action_idx = 0; action_idx < actions_.size(); ++action_idx) {
    if (action_idx_set.find(action_idx) == action_idx_set.end()) {
      prunedActions.push_back(actions_[action_idx]);
    }
  }
  assert(prunedActions.size() > 0);

  // Renormalize probabilities
  double sum_bp_prob = 0.0;
  double sum_next_prob = 0.0;
  for (const ActionData &action_data : prunedActions) {
    sum_bp_prob += action_data.bp_prob;
    sum_next_prob += action_data.next_prob;
  }
  for (ActionData &action_data : prunedActions) {
    action_data.bp_prob = sum_bp_prob > 0.0 ? action_data.bp_prob / sum_bp_prob
                                            : 1.0 / prunedActions.size();
    action_data.next_prob = sum_next_prob > 0.0
                                ? action_data.next_prob / sum_next_prob
                                : 1.0 / prunedActions.size();
  }
  actions_ = prunedActions;
}
...
void CFRStats::prune_action_idxs(const std::string &power,
                                 const std::vector<int> &action_idxs) {
  return power_stats_.at(power).prune_action_idxs(action_idxs);
}
