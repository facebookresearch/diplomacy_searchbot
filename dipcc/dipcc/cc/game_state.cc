...
const unordered_map<Power, unordered_set<Loc>> &
GameState::get_orderable_locations() {
  if (!orders_loaded_) {
...
void GameState::load_all_possible_orders_m() {
  ...
  orderable_locations_.reserve(units_.size());
  ...
  for (const auto &it : units_) {
    ...
    orderable_locations_[it.second.power].insert(root_loc(unit.loc));
...
void GameState::load_all_possible_orders_a() {
    ...
    if (n_builds_[p] > 0) {
      ...
      for (Loc center : home_centers_army(power)) {
        if (centers_.at(center) == power &&
          ...
          orderable_locations_[power].insert(root_loc(center));
        }
      }
  ...
  for (auto &p : units_) {
    if (can_disband[static_cast<int>(p.second.power) - 1]) {
      ...
      orderable_locations_[p.second.power].insert(root_loc(p.first));
    }
  }
}
...
GameState GameState::process(const unordered_map<Power, vector<Order>> &orders,
  ...
  if (phase_.phase_type == 'M') {
    ...
    JCHECK(false, "Invalid phase type: " + phase_.phase_type);
  }
}
