...
"""A service that can accept metrics from other workers and send them to tb/jsonl.
...
PORT_FILE = pathlib.Path("remote_logger_addr.txt")
...
class MetricLoggingServer:
    """A class that accepts metrics dicts as zmq and writes them to json/tb.
    """
            ...
            while True:
                ...
                if tag not in loggers_per_tag:
                    loggers_per_tag[tag] = Logger(
                        tag=tag or None, is_master=True
                    )
