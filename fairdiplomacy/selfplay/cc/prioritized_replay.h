...
private:
  ...
  SampleWeightIds sample_(int batchsize, const std::string &device) {
    return shuffle_cross_chunks_ ? sample_shuffled_(batchsize, device)
                                 : sample_chunk_(batchsize, device);
  }
  SampleWeightIds sample_chunk_(int batchsize, const std::string &device) {
  ...
  SampleWeightIds sample_shuffled_(int batchsize, const std::string &device) {
    std::unique_lock<std::mutex> lk(m_sampler_);
