...
class H2HItem:
    ...
    #       ("dipnet", "dipnet.temperature=0.5", "dipnet.model_path=/tmp/path")
    ...
    # Overrides for launcher. Put things like {"num_gpus": 2} here. See SlurmLauncher proto.
    launcher_overrides: Dict[str, Any] = dataclasses.field(default_factory=dict)
...
class H2HSweep:
    ...
    def maybe_launch(self, exp: H2HItem) -> fairdiplomacy.compare_agents_array.EvalRun:
        ...
        overrides = []
        launcher_overrides = self.SLURM_DEFAULTS.copy()
        launcher_overrides.update(exp.launcher_overrides)
        for k, v in launcher_overrides.items():
            overrides.append(f"launcher.slurm.{k}={v}")
